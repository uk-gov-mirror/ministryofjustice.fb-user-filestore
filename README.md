...
`curl -X POST --header "x-access-token: #{JWT.encode(payload.merge(checksum: Digest::SHA256.hexdigest(payload.to_json), iat: Time.now.to_i), 'service-token', 'HS256')}" --header "Content-Type: application/JSON" --data '#{payload.to_json}' http://localhost:3000/service/some-service/user/some-user`
...
response = `curl -X GET --header "x-access-token: #{JWT.encode(payload.merge({checksum: Digest::SHA256.hexdigest('')}), 'service-token', 'HS256')}" http://localhost:3000/service/some-service/user/some-user/28d-e71c352d0852ab802592a02168877dc255d9c839a7537d91efed04a5865549c1?payload=#{query_string_payload}`
